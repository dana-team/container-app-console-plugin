...
Additional parameters can be specified if desired. Consult the chart [values](charts/container-app-console-plugin/values.yaml) file for the full set of supported parameters.
...
```shell
helm upgrade -i  my-plugin charts/container-app-console-plugin -n my-namespace --create-namespace --set plugin.image=my-plugin-image-location
```
