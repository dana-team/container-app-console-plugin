...
export const commonReducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case 'setConfigurationImage':
      return {
        ...state,
        configuration: { ...state.configuration, image: action.value },
      };

    case 'setConfigurationContainerName':
      return {
        ...state,
        configuration: { ...state.configuration, name: action.value },
      };
    ...
    case 'addEnvVar':
      return {
        ...
        configuration: {
          ...state.configuration,
          env: [...state.configuration.env, { key: '', value: '' }],
        },
      };
      ...
      return {
        ...
        configuration: {
          ...
          env: state.configuration.env.map((env, idx) =>
            idx === action.index ? { ...env, key: action.value } : env,
          ),
        },
      };
